...
if (!$password || !$newpassword || !$confirmedpassword || $user_id <= 0) {
    ...
    header("Location: changePassword.php?user_id=$user_id&error=Invalid input!");
    exit();
}
...
if (!$user || !password_verify($saltedPassword, $user['password'])) {
    ...
    header("Location: changePassword.php?user_id=$user_id&error=Incorrect password!");
    exit();
}
...
if ($newpassword !== $confirmedpassword) {
    ...
    header("Location: changePassword.php?user_id=$user_id&error=Passwords do not match!");
    exit();
}
